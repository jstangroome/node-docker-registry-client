...
var HttpClient = require('restify/lib/clients/http_client');
    ...
    Object.defineProperty(this, '_httpapi', {
        get: function () {
            if (self.__httpapi === undefined) {
                self.__httpapi = new HttpClient(common.objMerge({
                    url: self._url
                }, self._commonHttpClientOpts));
...
RegistryClientV2.prototype.supportsV2 = function supportsV2(cb) {
    ...
    this.ping(function (err, body, res) {
        // XXX: Need to handle err here, for things like 'Error: getaddrinfo ENOTFOUND'.
        //if (err) {
        //    cb(err, false);
        //    return;
        //}
 ...
 *   function (err, res, digest, location, size) {
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function doLogin(_, next) {
                ...
                var scope = _makeAuthScope(resource, self.repo.remoteName, actions);
                self.login({
                    scope: scope
                }, next);
            },
            function _putManifest(_, next) {
                ...
                if (self._authInfo) {
                    headers.authorization = _authHeaderFromAuthInfo(self._authInfo);
                }
                self._httpapi.put({
                    path: fmt('/v2/%s/manifests/%s',
                        encodeURI(self.repo.remoteName),
                    ...
                    req.on('result', function onReqResult(reqErr, res_) {
                        ...
                        res_.on('end', function onResEnd() {
                            // Parse and handle errors in the body of the response.
                            ...
                            size = parseInt(res_.headers['content-length'], 10);
                            res = res_;
                            next();
                        });
                    });
        ...
        cb(err, res, digest, location, size);
    });
};
...
/*
 * Initiate a blob upload. If successful, an upload location will be
 * provided to complete the upload. Optionally, if the `digest`, `stream` and
 * `contentLength` parameters are present, the request stream will be used to
 * complete the upload in a single request.
 *
 * <https://docs.docker.com/registry/spec/api/#starting-an-upload>
 *
 *   client.startBlobUpload({digest: <string>, stream: <object>,
 *      contentLength: <number>},
 *   function (err, res) { });
 */
RegistryClientV2.prototype.startBlobUpload = function startBlobUpload(opts, cb) {
    var self = this;
    assert.object(opts, 'opts');
    assert.optionalNumber(opts.contentLength, 'opts.contentLength');
    assert.optionalString(opts.contentType, 'opts.contentType');
    assert.optionalString(opts.digest, 'opts.digest');
    assert.optionalObject(opts.log, 'opts.log');
    assert.optionalObject(opts.stream, 'opts.stream');
    assert.func(cb, 'cb');
    if (opts.digest) {
        // When using a digest, a contentLength and stream must be provided.
    ...
    vasync.pipeline({
        ...
        funcs: [
            function doLogin(_, next) {
                log.trace({digest: opts.digest}, 'startBlobUpload:: doLogin');
                var resource = 'repository';
                var actions = ['pull', 'push'];
                //var actions = ['push'];
                var scope = _makeAuthScope(resource, self.repo.remoteName, actions);
            ...
            function _getUploadUuid(_, next) {
                log.trace({digest: opts.digest},
                    'startBlobUpload:: _getUploadUuid');
                var path = fmt('/v2/%s/blobs/uploads/',
                    encodeURI(self.repo.remoteName));
                var headers = {};
                if (self._authInfo) {
                    authHeader = _authHeaderFromAuthInfo(self._authInfo);
            ...
            function _putBlob(_, next) {
                log.trace({digest: opts.digest}, 'startBlobUpload:: _putBlob');
                var path = uploadUrl;
                if (opts.digest) {
                    var urlSep = '?';
                    if (path.indexOf('?') > 0) {
                    ...
                    path = fmt('%s%sdigest=%s', path, urlSep,
                ...
                self._httpapi.put({
                    ...
                    log.trace({digest: opts.digest}, 'startBlobUpload:: _afterCall');
                    if (err) {
                        next(err);
                        return;
                    }
                    ...
                    function onReqResult(reqErr, res_) {
                        log.trace({digest: opts.digest}, 'startBlobUpload:: onResult');
                        res_.body = '';
                        res_.on('data', function onResChunk(chunk) {
                            res_.body += chunk;
                        });
                    ...
                    stream.pipe(req);
                    log.trace({digest: opts.digest}, 'startBlobUpload:: piped stream to req');
                });
            }
        ]
    }, function _startBlobUploadCb(err) {
        cb(err, res);
    });
};
